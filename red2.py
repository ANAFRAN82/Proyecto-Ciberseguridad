...
def capturar_y_enviar():
    ...
    def captura():
        try:
            ...
            # Mostrar contenido completo de cada paquete
            for i, paquete in enumerate(paquetes, 1):
                ...
                # Mostrar detalles completos del paquete
                resultado.insert(tk.END, f"Resumen completo:\n{paquete.show(dump=True)}\n")
            
            # Guardar paquetes localmente
            nombre_archivo = f"captura_remota_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pcap"
            wrpcap(nombre_archivo, paquetes)
            resultado.insert(tk.END, f"[✔] Captura guardada localmente en {nombre_archivo}\n")
...
# Campos de entrada
tk.Label(ventana, text="Sniffer").grid(row=0, column=0, columnspan=2, pady=10)
...
# Botón para iniciar
tk.Button(ventana, text="Iniciar Captura", command=capturar_y_enviar).grid(row=3, column=0, columnspan=2, pady=10)

# Área de resultados
tk.Label(ventana, text="Resultados:").grid(row=4, column=0, columnspan=2)
